...
class BranchAndBoundSolver:
    ...
    def _bb(self, last_node, visited, cost, battery_left, order):
        ...
        if len(visited) == len(self.clients):
            ...
            final_cost = (cost[0] + distance, cost[1] + risk, cost[2] + recharges_used)
        ...
        for client in self.clients:
            ...
            distance, risk, recharges_used, new_battery = result
            new_cost = (cost[0] + distance, cost[1] + risk, cost[2] + recharges_used)
    ...
    def _is_dominated(self, last_node, visited, cost, battery_left) -> bool:
        ...
        for current_cost, current_battery in pareto_frontier:
            ...
            dominates = (
                ...
                and current_risk <= new_risk
                and current_recharges <= new_recharges
                and current_battery >= battery_left
                and (
                    current_distance < new_distance
                    or current_risk < new_risk
                    or current_recharges < new_recharges
                    or current_battery > battery_left
                )
            )
    ...
    def _register_pareto(self, last_node, visited, cost, battery_left) -> None:
        ...
        for current_cost, current_battery in pareto_frontier:
            ...
            new_dominates_current = (
                new_distance <= current_distance
                and new_risk <= current_risk
                and new_recharges <= current_recharges
                and battery_left >= current_battery
                and (
                    new_distance < current_distance
                    or new_risk < current_risk
                    or new_recharges < current_recharges
                    or battery_left > current_battery
                )
            )
    ...
    def _try_complete_solution(self, order, cost):
        ...
        for existing_order, (d, r, rc) in self.best_solutions:
            if not (
                (distance <= d)
                and (risk <= r)
                and (recharges_used <= rc)
                and (distance < d)
                or (risk < r)
                or (recharges_used < rc)
            ):
