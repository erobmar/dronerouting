...
def main() -> None:
    ...
    for json_path in instances:
        ...
        print(f"\n--- {json_path.name} ---")
        print(f"hub = {hub} | clients = {len(graph.clients)} | start_battery = {start_battery}")
        ...
        if sa_init_order is not None:
            (sa_res, sa_time) = timed(simulated_annealing,
                                      graph,
                                      hub,
                                      start_battery,
                                      sa_init_order,
            ...
            add_row(rows, json_path.name, len(graph.clients), "simulated_annealing", sa_res is not None, sa_time, sa_order, sa_cost)
            print(f"SA:              ok={sa_res is not None} time={sa_time:.4f}s cost={sa_cost}")
        else:
        ...
        if len(graph.clients) <= 12: # Ajustar este umbral en función del equipo en que se ejecute
            ...
            if bb_frontier_size > 0:
                bb_best_cost = min((cost for _ , cost in bb_solutions), key=lambda c: c[0])
            
            add_row(rows, json_path.name, len(graph.clients), "exact_bb", bb_frontier_size > 0, bb_time, None, bb_best_cost, frontier_size=bb_frontier_size)           
            print(f"BB exact:          frontier={bb_frontier_size} time={bb_time:.4f}s best_cost={bb_best_cost}")
        else:
