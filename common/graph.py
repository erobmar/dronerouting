...
class Graph:
    ...
    def transfer(self, start: str, goal: str, battery_left: float):
        ...
        while priority_queue:
            ...
            recharges_used, distance_acc, risk_acc, current_node, battery_remaining = (
            ...
            if (
                previous_best_battery is not None
                and previous_best_battery >= battery_remaining
            ):
                continue
...
def build_from_json(path: str) -> Graph:
    ...
    # Extraemos los nodos
    nodes = {
        node["id"]: Point(float(node["x"]), float(node["y"])) for node in data["nodes"]
    }
    ...
    graph_from_json = Graph(
        float(data["max_battery"]),
        nodes,
        clients,
        recharges,
        hub,
